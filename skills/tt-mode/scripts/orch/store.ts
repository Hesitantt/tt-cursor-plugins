...
async function readLedger(store: string): Promise<readonly LedgerEntry[]> {
	return (await readTsv(join(store, 'ledger.tsv'), LEDGER_HEADER, 6)).map(
		(row) => {
			...
			if (verdict === null) {
				throw new UserError(
					`ledger.tsv has invalid verdict ${rawVerdict}`,
				)
			}
			return {
				pr: row[0] ?? '',
...
function parseJsonValue(raw: string, source: string): unknown {
	try {
		...
		throw new UserError(`${source} returned invalid JSON`)
	}
}
...
function parseDefaultBranch(raw: string): string {
	const parsed = parseJsonValue(raw, 'gh repo view')
	if (!isRecord(parsed) || !isRecord(parsed.defaultBranchRef)) {
		throw new UserError('gh repo view returned an invalid shape')
	}
...
function parseFrontierState(value: unknown): FrontierPrState {
	if (typeof value !== 'string') {
		throw new UserError('gh pr list returned a pull request with no state')
	}
	const state = value.toUpperCase()
	if (state === 'OPEN' || state === 'MERGED' || state === 'CLOSED') {
		return state
	}
	throw new UserError(`gh pr list returned an unknown pull request state: ${value}`)
}

function parseGhPullRequest(row: unknown): GhPullRequest {
	if (!isRecord(row)) {
		throw new UserError('gh pr list returned a non-object pull request')
	}
	const number = row.number
	if (typeof number !== 'number' || !Number.isSafeInteger(number) || number < 1) {
		throw new UserError('gh pr list returned a pull request with an invalid number')
	}
	...
	if (typeof headRefName !== 'string' || headRefName.length === 0) {
		throw new UserError(`gh pr list returned pull request #${number} with no head branch`)
	}
	if (typeof baseRefName !== 'string' || baseRefName.length === 0) {
		throw new UserError(`gh pr list returned pull request #${number} with no base branch`)
	}
	if (typeof headRefOid !== 'string' || !/^[0-9a-f]{40,64}$/i.test(headRefOid)) {
		throw new UserError(`gh pr list returned pull request #${number} with an invalid SHA`)
	}
	return {
		number,
		state: parseFrontierState(row.state),
		headRefName,
		baseRefName,
		headRefOid,
	}
...
function parseGhPullRequests(raw: string): readonly GhPullRequest[] {
	const parsed = parseJsonValue(raw, 'gh pr list')
	if (!isUnknownArray(parsed)) {
		throw new UserError('gh pr list returned an invalid shape')
	}
	return parsed.map((row) => parseGhPullRequest(row))
}

function addGhPullRequest({
	byHead,
	pr,
}: {
	byHead: Map<string, GhPullRequest>
	pr: GhPullRequest
}): void {
	const existing = byHead.get(pr.headRefName)
	if (existing !== undefined && existing.number !== pr.number) {
		throw new UserError(
			`gh listed multiple pull requests for branch ${pr.headRefName}`,
		)
	}
	byHead.set(pr.headRefName, pr)
}

function fetchGhPullRequestByHead({
	branch,
	repo,
}: {
	branch: string
	...
	const found = parseGhPullRequests(
		runGh({
			repo,
			args: [
				'pr',
	...
	if (found.length === 0) {
		throw new UserError(`gh found no pull request for branch ${branch}`)
	}
	if (found.length > 1) {
		throw new UserError(`gh listed multiple pull requests for branch ${branch}`)
	}
	const pr = found[0]
	if (pr === undefined) {
		throw new UserError(`gh found no pull request for branch ${branch}`)
...
function collectGithubStack({
	repo,
	trunk,
}: {
	repo: string
	trunk: string
}): readonly GhPullRequest[] {
	const byHead = new Map<string, GhPullRequest>()
	const open = parseGhPullRequests(
		runGh({
			repo,
			args: [
				'pr',
	...
	for (const pr of open) {
		addGhPullRequest({ byHead, pr })
	}
	const pending = open.map((pr) => pr.baseRefName)
	while (pending.length > 0) {
		const base = pending.pop()
		if (base === undefined || base === trunk || byHead.has(base)) {
			continue
		}
		const parent = fetchGhPullRequestByHead({ branch: base, repo })
		addGhPullRequest({ byHead, pr: parent })
		pending.push(parent.baseRefName)
	}
	return [...byHead.values()]
}

function orderGithubStack({
	prs,
	trunk,
}: {
	prs: readonly GhPullRequest[]
	trunk: string
}): readonly GhPullRequest[] {
	...
	function walk(pr: GhPullRequest): void {
		if (visited.has(pr.number)) {
			return
		}
		if (visiting.has(pr.number)) {
			throw new UserError(
				`gh pull request chain contains a cycle at #${pr.number}`,
			)
		}
		visiting.add(pr.number)
		if (pr.baseRefName !== trunk) {
			const parent = byHead.get(pr.baseRefName)
			if (parent === undefined) {
				throw new UserError(
					`gh pull request #${pr.number} bases on ${pr.baseRefName}, which has no pull request`,
				)
			}
			walk(parent)
		}
		visiting.delete(pr.number)
		visited.add(pr.number)
		ordered.push(pr)
	}
	const numbered = [...prs].sort((left, right) => left.number - right.number)
	for (const pr of numbered) {
		walk(pr)
	}
	return ordered
}
...
function resolveFrontier(repo: string): readonly FrontierPr[] {
	const trunk = parseDefaultBranch(
		runGh({
			repo,
			args: ['repo', 'view', '--json', 'defaultBranchRef'],
		}),
	)
	const prs = orderGithubStack({
		prs: collectGithubStack({ repo, trunk }),
		trunk,
	})
	if (new Set(prs.map((row) => row.number)).size !== prs.length) {
		throw new UserError('gh listed duplicate pull request numbers')
	}
	return prs.map((row) => ({
		pr: row.number,
	...
	return {
		...
		frontier: {
			set: async (params) => {
				await beginWrite()
				const repo = resolve(
					requiredLine(params.repo, 'repo directory'),
				)
				const pin =
				...
				const value: Frontier = {
					...
					prs,
					lowestUnmerged:
						prs.find((row) => row.state === 'OPEN')?.pr ?? null,
				}
				await atomicWrite(
