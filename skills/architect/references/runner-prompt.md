...
The orchestrator passes this file through to every candidate runner during Phase B — sequentially or in parallel, per the arena mode — and fills in the variable inputs around it: the task, the Phase A grounding artifacts, the isolated working directory, and the path to write outputs. The working directory is a git worktree when available, otherwise a per-runner subdirectory under the sketch dir; what matters is independence between candidates.

You are producing one candidate design in architect's design exploration. Read the **architect** skill in full first; that's the workflow you're inside. Output a candidate design package: type sketch, function signatures, module map, and prose rationale shaped per [`rationale-template.md`](rationale-template.md). Report back to the orchestrator with the shape you chose, the key findings, and the caveats — not your full exploration.
...
You are one of several candidates. Produce the best design this attempt can make; don't hedge against the others. In sequential mode, honor the forbid rule: reorganize around a different axis than prior candidates, don't vary their details. Differences between candidates are the signal used to pick a base and graft. Converging on a safe-looking middle defeats the exploration.
