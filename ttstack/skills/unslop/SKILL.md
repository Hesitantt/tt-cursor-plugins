...
13. **Em dash overuse.** Avoid em dashes entirely. Use periods or commas only (no parentheses, no em dashes, no en dashes, no hyphen-as-dash substitutes). Em dashes are an AI tell, and reaching for parentheses instead just trades one tell for another. If a thought needs separation, end the sentence or use a comma.
