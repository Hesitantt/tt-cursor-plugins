...
Do not wrap this pass in `/swarm`. Source readers are the `maintain-source-readers` role, not `swarm-workers` and not arena candidates. The coordinator owns every live drive.
