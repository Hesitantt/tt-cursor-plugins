...
2. The matching playbook from `references/source-playbook.md` when one exists for that category. Otherwise adapt the nearest example in that index to the selected MCP. Do not invent a `references/sources/<name>.md` path that is not in the index.
...
2. **Issue / ticket tracker investigator**. Read `docs/issue-tracker.md` in the working repo and use the tracker it names. Best at surfacing *the product or business forcing function*. Strongest when the why is external to engineering.
