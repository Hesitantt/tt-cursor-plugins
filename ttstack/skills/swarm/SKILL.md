...
4. Pick the worker model from the `swarm-workers` line in the User Rule titled `ttstack models`. If that rule or line is missing, or the value is `inherit-parent` or `auto`, omit `model`. For a model race, name each arm's model up front.
