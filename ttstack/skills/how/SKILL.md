...
- `model`: the `how-explorer` line from the User Rule titled `ttstack models` (omit `model` if missing, `inherit-parent`, or `auto`)
- `readonly`: `true`
...
- `subagent_type`: `generalPurpose`
- `model`: the `how-explainer` line from the User Rule titled `ttstack models` (omit `model` if missing, `inherit-parent`, or `auto`)
- `readonly`: `true`
...
After the explanation is complete, spawn one architectural critic per entry in the `how-critics` list from the User Rule titled `ttstack models`, all in a single message. If that rule or line is missing, spawn one critic and omit `model`.

For each critic:
- `subagent_type`: `generalPurpose`
- `model`: that list entry (omit `model` if `inherit-parent` or `auto`). These are minimum reasoning levels. The lead should escalate any model when the architecture warrants deeper analysis.
- `readonly`: `true`
