...
const h2 = (l) =>
	!l.code && l.text.startsWith('## ') ? l.text.slice(3).trim() : null
const sections = []
...
const boxes = (ls) =>
	ls
		.filter((l) => !l.code && BOX.test(l.text))
		.map((l) => ({ n: l.n, text: l.text.match(BOX)[1] }))
...
if (h1 !== -1 && howToRead) {
	const intro = lines
		.slice(h1 + 1)
		.filter((l) => l.n < howToRead.n && l.text.trim() !== '')
	if (intro.length >= 10)
		fail(lines[h1].n, `intro is ${intro.length} lines, under ten required`)
...
if (!program) fail(1, 'no "## Program checklist" section')
	...
	for (const name of PROGRAM_H3) {
		...
		if (at === -1)
			fail(program.n, `Program checklist lacks "### ${name}" in order`)
		else cursor = at + 1
	}
	for (const marker of PROGRAM_MARKERS) {
		const ok =
			marker instanceof RegExp
				? marker.test(bodyText(program))
				: bodyText(program).includes(marker)
...
for (const pr of prSections) {
	...
	if (names.join('|') !== SUB_BLOCKS.join('|')) {
		fail(
			pr.n,
			`${pr.title}: sub-blocks are [${names.join(', ')}], expected [${SUB_BLOCKS.join(', ')}]`,
		)
	}
	...
	for (const name of [
		...
		const b = block(name)
		if (b && boxes(b.lines).length === 0)
			fail(b.n, `${pr.title}: ${name} has no box`)
	}
	for (const name of ['Verify, unit.', 'Verify, live.', 'Verify, perf.']) {
		const b = block(name)
		if (b && !b.rest.startsWith(RULE))
			fail(b.n, `${pr.title}: ${name} does not open with the rule`)
	}

	const live = block('Verify, live.')
	if (live) {
		if (!live.rest.includes(LANES))
		...
		for (const lane of lanes) {
			if (!lane.m) fail(lane.n, `${pr.title}: live box is not a lane`)
				...
				fail(
					lane.n,
					`${pr.title}: lane ${lane.m[1]} names no screenshot`,
				)
			else if (!lane.text.includes('Pass when'))
				fail(
					lane.n,
					`${pr.title}: lane ${lane.m[1]} has no pass predicate`,
				)
		}
	}
	...
	if (perf) {
		...
		if (items.join('|') !== PERF_ITEMS.join('|'))
			fail(
				perf.n,
				`${pr.title}: perf boxes are [${items.join(', ')}], expected [${PERF_ITEMS.join(', ')}]`,
			)
	}
	...
	if (gate) {
		...
		if (gate.rest.startsWith('None.')) {
			if (gateBoxes.length)
				fail(gate.n, `${pr.title}: Review gate says None but has boxes`)
		} else {
			const text = gate.lines.map((l) => l.text).join('\n')
			if (gateBoxes.length === 0)
				fail(gate.n, `${pr.title}: Review gate has no box`)
			for (const word of ['screenshot', 'video', 'operator']) {
				if (!text.includes(word))
					fail(gate.n, `${pr.title}: Review gate lacks "${word}"`)
			}
		}
	}
...
if (closeIndex !== -1) {
	...
	for (const s of tail) {
		if (!s.title.startsWith('Appendix'))
			fail(
				s.n,
				`"## ${s.title}" after Close the program is not an appendix`,
			)
	}
