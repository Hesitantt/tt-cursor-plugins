...
function createProgram(io: Io): Command {
	...
	const frontier = program
		.command('frontier')
		.description('manage the merge frontier (GitHub-only via gh)')
		.action(() => requireSubcommand(program))
	leaf(
		frontier,
		'set',
		'discover GitHub stacks with gh and write frontier.json',
	)
		.addOption(
			new Option(
				'--repo <dir>',
	...
	leaf(standing, 'add <line>', 'add a standing order').action(
		(line: string) =>
			runStore(
				program,
				io,
